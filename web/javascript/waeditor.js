...
function onTourLoad(data){
	...
	var box = window.innerHeight * 0.06;
	crosshairs.height(box)
	crosshairs.width(box);
	crosshairs.css({left:window.innerWidth/2 - box/2,top:window.innerHeight/2 - box/2});
	animate();
...
function setScene(index,first,keepDirection,end_angle){
	...
	if(s.room_ref && !keepDirection){
		...
		lastDoor = -1;
		if(s.room_ref.usefov){
			myviewer.camera.fov = s.room_ref.fov;
			myviewer.camera.updateProjectionMatrix()
		}
	}
	listDoors(s.doors);
...
function roomReference(gotoNext){
	...
	if(editor.useScaleDistance()){
		scene.room_ref = {lat:myviewer.lat,lon:myviewer.lon,usefov:true,fov:myviewer.camera.fov,distance: distance};						
	}
	else {
...
function animate() {
	...
	if(editor.zoom){
		/*
		var phi = THREE.Math.degToRad( 90 - myviewer.lat );
		var theta = THREE.Math.degToRad( myviewer.lon );
		//console.debug(this.lat + " " + this.lon);


		myviewer.base.x = editor.zoom * Math.sin( phi ) * Math.cos( theta );
		myviewer.base.y = editor.zoom * Math.cos( phi );
		myviewer.base.z = editor.zoom * Math.sin( phi ) * Math.sin( theta );
		*/
	}

}

editor.useScaleDistance = function(){
	return $('#scaledistance').is(":checked");
};

function calculateDistance(height){
	if(editor.useScaleDistance()){
		var degrees = (180 - (myviewer.camera.fov * 0.06))/2;
		var adjacent = 1;
