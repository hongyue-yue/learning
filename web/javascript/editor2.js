...
function onTourLoad(data){
	...
	var iframe = document.querySelector('#mainscene');
	mainsphere = iframe.contentWindow.myviewer;
	//console.debug("HASDFASDF"+mainsphere);
...
function updateTinyPlanet(){
	...
	tpsphere.setImage(tourManager.get_url(nexts.panorama),nexts.north,mainsphere.lon);		
}
...
function moveAround(e){
	...
	if(c=='110'){
		...
		console.debug(mainsphere.lat + " " + calculateDistance(6));
		roomReference();
		//console.debug(mainsphere.lat+ " "+ mainsphere.lon);
	}
	else if(c=='104'){
		var distance = calculateDistance(6);
		console.debug("Heading "+distance + " " + mainsphere.lon);
	}
	else if(c=='100'){
		doorReference();
		console.debug("Heading "+distance + " " + mainsphere.lon);
...
function setNorth(){
	var north = mainsphere.lon;
	if(tour_scenes[pathi].north){
		north = (tour_scenes[pathi].north + north) % 360;
	}
	tour_scenes[pathi].north = north;
	mainsphere.mesh.rotation.y = THREE.Math.degToRad(north);
	mainsphere.lon = 0;
	console.debug("Heading "+north + " " + mainsphere.lon);
...
function moveForwards(){
	...
	var closest = sceneUtil.findClosestPath(scene, mainsphere.lon);
	if(closest==null){
		return;
	}
	if(Math.abs(closest.diff) > 30){
		if(closest.diff < 0){
			mainsphere.lon = (360 + mainsphere.lon + 4) % 360;
		}
		else {
			mainsphere.lon = (360 + mainsphere.lon - 4) % 360;			
		}
...
function moveBackwards(){
	var scene = tour_scenes[pathi];
	var closest = sceneUtil.findClosestPath(scene, (mainsphere.lon + 180) % 360);
	if(closest==null){
		return;
	}
...
function setScene(index,first,keepDirection,end_angle){
	...
	mainsphere.setImage(tourManager.get_url(s.panorama),s.north,end_angle);
	...
	if(s.room_ref && !keepDirection){
		mainsphere.lat = s.room_ref.lat;
		mainsphere.lon = s.room_ref.lon;
		lastDoor = -1;
	}
	listDoors(s.doors);
...
function addRefPoint(){
	...
	console.debug("Heading "+distance + " " + mainsphere.lon);
	// x=5cos,and y=5sin
	var x = (distance * Math.cos(mainsphere.lon * (Math.PI/180))) + scene.coords.x;
	var y = (distance * Math.sin(mainsphere.lon * (Math.PI/180))) + scene.coords.y;
...
function tiePoints(){
	...
	console.debug("Heading "+distance + " " + mainsphere.lon);
	// x=5cos,and y=5sin
	mainsphere.lon = (mainsphere.lon +720) % 360;
	var x = distance * Math.cos(mainsphere.lon * (Math.PI/180));
	var y = distance * Math.sin(mainsphere.lon * (Math.PI/180));
...
function roomReference(gotoNext){
	...
	scene.room_ref = {lat:mainsphere.lat,lon:mainsphere.lon};
	console.debug(JSON.stringify(scene.coords));
...
function doorReference(useTPID,trail){
	...
	var data = {lat:mainsphere.lat,lon:mainsphere.lon, join: doorID};
	...
	if(data!=null){
		scene.doors.push({lat:mainsphere.lat,lon:mainsphere.lon, join: doorID, trail:true});		
	}
	
	console.debug("HMMM "+doorID+" "+tour_scenes[pathi].sceneid);
	listDoors(scene.doors);
	console.debug(JSON.stringify(scene.coords));
	goThruDoor(doorID,mainsphere.lat,mainsphere.lon);
}
...
function animate() {
	...
	//mainsphere.update();
	if(tpsphere){
		tpsphere.controls.setThetaPhi(mainsphere.controls.getThetaPhi());
		tpsphere.lat = mainsphere.lat;
		tpsphere.lon = mainsphere.lon;
		if(tpsphere.camera.fov!=mainsphere.camera.fov){
			tpsphere.camera.fov=mainsphere.camera.fov;
		...
		tpsphere = iframe.contentWindow.mainsphere;
...
function calculateDistance(height){
	var adjacent = height;
	var rads = (90 + mainsphere.lat) * (Math.PI/180);
...
function debugNav(){
	var scene = tour_scenes[pathi];
	console.debug(mainsphere.lon+" "+JSON.stringify(scene.paths));
}
